...
from models import SessionLocal, User, init_db, ensure_schema
...
def to_user_timezone(dt, timezone_str):
    if not dt:
        return dt
    try:
...
def parse_reminder_time(text):
    formats = ["%d/%m/%Y %I:%M %p", "%d/%m/%Y %H:%M", "%d/%m/%Y %I:%M%p", "%d/%m/%Y %H%M"]
    for fmt in formats:
        try:
...
async def process_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if flow == "add_reminder_datetime":
        reminder_dt = parse_reminder_time(text)
        if not reminder_dt:
...
async def reminder_dispatcher(context: ContextTypes.DEFAULT_TYPE):
    ...
    due_reminders = get_due_reminders(now_utc)
    for reminder in due_reminders:
        ...
        local_time = from_utc(reminder.reminder_datetime)
        message = (
            f"Reminder time is on.\n\n"
            f"Task: {reminder.description}\n"
            f"Scheduled time: {local_time.strftime('%I:%M %p')}\n"
